...
int main (){
    ...
    int i, j, option, burgerCount, currentBC, receiptNumber=0, last;    //declaring variables
    float disscount, burgerCost, currentBill;
    char burgerType, addMore;
    ...
    for (i=0; i<SIZE; i++){
        ...
        if (option == 1){
            ...
            do{
                ...
                burgerCount += currentBC; //calculate current burger count and store in burgerCount variable
                currentBill += burgerCost * currentBC;  //calculate current bill
                printf("Your current bill value is $%.2f\n", currentBill);

                printf("\nPress 'Y' to add more burgers into your order or press 'N' to process your order: ");
                scanf(" %c%*c", &addMore);
                printf("\n");
        ...
        if (burgerCount >= 5){  //checking burger count greater than 5
            ...
            receipt[i].amount = currentBill - disscount;     //adding discount if burger count grater than 5
        ...
        currentBill = 0;    //values of current bill will be reset
